...
class Interfaz(Frame):
    ...
    def preguntaGuardar(self):
        if self.originalVentana.find_all() != ():
            ...
            self.label = Label (self.top, text= "Para guardar la imagen tienes que poner el nombre que le quieras poner seguido del formato de la imagen original.\n Ej: Si abriste \"perrito.jpeg\" puedes guardar la nueva imagen como \"perritonuevo.jpeg\"")
            self.label.pack()
    ...
    def guardarImagen(self):
        try:
            ...
            tkinter.messagebox.showwarning("Error","No puedes guardar una imagen en la que no se haya aplicado algún filtro")
        except ValueError:
    ...
    def aplicaFiltro(self,opcion):
        if self.originalVentana.find_all() != ():
            ...
            self.originalVentana.create_image(imageOtra.width()/2, imageOtra.height()/2, anchor=CENTER, image=imageOtra, tags="bg_img")
        else:
